...
int insertar_vertice(grafo *G, tipovertice Vert) {
    int i;
    if ((*G)->N == MAXVERTICES) {
    	// Se ha llegado al máximo número de vértices
    	return -1;
    }
    ...
    for (i = 0; i < (*G)->N; i++) {
        ...
        //Se inicializan los arcos entre el vértice creado y los otros vértices a 0 en la matriz AI
        (*G)->AI[i][((*G)->N) - 1] = 0;
...
void borrar_vertice(grafo *G, tipovertice Vert) {
    ...
    //if (P >= 0 && P < (*G)->N) {
    for (F = P; F < N - 1; F++){
        (*G)->VERTICES[F] = (*G)->VERTICES[F + 1];   //Se mueven los vértices a la izquierda en el array que los guarda
	}
    for (C = P; C < N - 1; C++){
        for (F = 0; F < N; F++){
