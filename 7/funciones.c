...
//Opción p del menú, calcular las conexiones mínimas que mantienen conectadas las habitaciones
...
//Opción r del menú, calcular la ruta más corta entre dos habitaciones dadas
void ruta_minima(grafo G) {
    ...
    if (!existe_vertice(G, orig)) {
        printf("La habitacion %s no existe en el mapa\n", orig.habitacion);
        return;         //Tras mostrar un mensaje por pantalla, se vuelve al menú
    }
    ...
    if (strcmp(orig.habitacion, dest.habitacion) == 0) {       //En este caso, se puede ahorrar la llamada a _floyd
        printf("La habitación de origen y la de destino coinciden. La distancia total es \033[0;32m0\033[0m\n");
        printf("\tRuta: \033[0;36m%s\033[0m", dest.habitacion);
    } else {
        _floyd(G, posicion(G, orig), posicion(G, dest), rol);     //Se calcula la ruta más corta
    }
}
