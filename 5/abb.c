...
tipoclave _clave_elem(tipoelem *E) {
    return E->nombreJugador;        //La clave es el nombre del jugador
}
...
void _destruir_elem(tipoelem *E) {
    if (E->tareas != NULL) {            //Se comprueba que la cola no haya sido previamente destruida
        destruir_cola(&E->tareas);              //Se destruye la cola
        //No es necesario poner E->tareas a NULL por la construcción del programa
    }
}
