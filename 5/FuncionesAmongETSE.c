...
void _estadoJugadores(abb A){
    ...
    if (!es_vacio(A)){
        ...
        //Si el jugador está vivo o ha muerto este turno, se imprimirá la habitación en la que se encuentra
        if (E.tareas != NULL && !es_vacia_cola(E.tareas)){
            //Si la cola de tareas fue destruida, apunta a NULL. No se puede llamar a es_vacia_cola() directamente
            EC = primero(E.tareas);
            printf(" situado en: \033[0;36m|%-15s|\033[0m", EC.lugarTarea);
...
void _contarTripulantesHabitacion(abb A, char *hab, int *num){
    ...
    if (!es_vacio(A)){
        ...
        if (E.tareas != NULL && !es_vacia_cola(E.tareas)){
            //Se debe comprobar que la cola de tareas no haya sido destruida antes de llamar a es_vacia_cola()

            EC = primero(E.tareas);                 //Se lee el primer elemento de la cola
            if (E.rol == 'T' && strcmp(EC.lugarTarea, hab) == 0){
...
void _ejecutarTarea(abb A, abb B, int *contador, int nImpostores, int *sabotaje){
    tipoelem E;
    tipoelemCola EC;
    ...
    if (!es_vacio(A)){
        ...
        if (E.rol == 'I'){
            ...
            if (!es_vacia_cola(E.tareas)){
                //No es necesario comprobar que E.tareas != NULL porque entonces el jugador tendría como rol E o K
                EC = primero(E.tareas);
                _contarTripulantesHabitacion(A, EC.lugarTarea, &num);
                //Se comprueba cuántos tripulantes hay en su habitación
            }
            if(num > 0){        //Hay al menos un tripulante
                ...
                if (intervencion) {         //Los fantasmas no han intervenido. Un tripulante de la habitación muere
                    ...
                    _kill(A, EC.lugarTarea, elegido, &num);     //El impostor mata al jugador elegido
                }

            } else {        //num == 0, el impostor está solo
                _sabotear(A, B);
                *sabotaje = 1;      //Se indica que ha ocurrido un sabotaje (ahora la partida tendrá 1 turno más)
            }
        }
        ...
        if (*contador < nImpostores){
            _ejecutarTarea(izq(A), B, contador, nImpostores, sabotaje);
        }

        if (*contador < nImpostores){
            _ejecutarTarea(der(A), B, contador, nImpostores, sabotaje);
        }
...
void _adivinarJugador(abb A, int *nImpostores){
    ...
    printf("Vote por un impostor: ");
    scanf("%s", votado.nombreJugador);

    while (!es_miembro(A, votado)){         //El nombre no es válido (no está en el árbol de jugadores)
        printf("No es un jugador de la partida\n");
        printf("Vote por un impostor: ");
        scanf("%s", votado.nombreJugador);
    }

    buscar_nodo(A, votado.nombreJugador, &votado);      //Se busca al jugador
    while (votado.rol == 'K'){                          //Si está muerto, se ofrece una oportunidad más para adivinar
...
void jugarPartida(abb *Arbol) {
    ...
    int i, fin, sabotaje, controlSabotajes = 0;             //Contadores y variables dependientes del usuario
    ...
    fin = NTAREAS;          //En un principio, solo habrá 5 turnos (a menos que ocurra un sabotaje)
    ...
    for (i = 0; i < fin; i++) {
        ...
        _ejecutarTarea(jugadores, *Arbol, &contador, nImpostores, &sabotaje);
        //Es necesario pasar Arbol por los sabotajes de los impostores
        //Si ocurre un sabotaje, _ejecutarTarea devolverá sabotaje como 1
        fin += sabotaje;    // Si tiene lugar un sabotaje, las colas de tareas tendrán un elemento más <-> 1 turno más
        ...
        //Se suprime la tarea actual y se destruye la cola de los jugadores muertos y eliminados
    }
